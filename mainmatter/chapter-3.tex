...
% TODO (Jasper, 27 Aug 2026) RESEARCH NEEDED: "Did you also look at the SuperSharp design?"
% Note the correction made in the meeting: SuperSharp and the SSTL concept are two DIFFERENT projects, both in the UK.
% SuperSharp is a thermal-infrared baffle design; the SSTL concept (cited in sec:agreement) is the one that integrates
% baffle and optics. The five SHARP papers are not the run-up to SuperSharp. Add SuperSharp here with a short description
% of what it is and what happened to it.
...
Each family is assessed against the properties of \Cref{sec:properties} and against the evidence that it has been built for an optical standoff. The outcome is whether to carry it into the design trade-off or to set it aside. There is no middle shelf: a family that is not modelled cannot be placed beside families that are without reproducing the incomparable comparison \Cref{sec:review-method} rejects. Where a family set aside is genuinely good at something, that is recorded with it.
...
\begin{itemize}
    \item Sub-micron class performance is reached with active correction. Every reviewed system that achieves it does so behind a correction loop, and no architecture is claimed to reach it passively. Villalba et al. \cite{villalba_review_2020} state this as the consensus of the deployable-optics literature, contrasting deployment repeatability of a few micrometres against wavefront-error requirements of tens of nanometres. The two are not the same budget and the comparison should not be read as one. Wavefront error is a property of a mirror surface with respect to its ideal figure, and it is the governing budget where the mirror itself is deployed in segments. What governs here is different: the placement of M2 with respect to M1 in its remaining degrees of freedom, for which the tolerance depends on the instrument, the wavelength and the resolution to be achieved, and typically falls at the micron or sub-micron level rather than the nanometre level. The question for this thesis is therefore how far passive design reaches against that placement budget and what the residual costs, rather than whether correction is needed at all.
    ...
    % TODO (Jasper, 27 Aug 2026) RESEARCH NEEDED. Go deeper on these integrated-baffle concepts: how mature are the designs,
    % and do they name accuracy and possible thermo-mechanical issues? The stakes: if they were never worked out or modelled
    % and do not discuss it, calling them naive is defensible. If they computed it and claim it is not a problem, our own
    % rejection assumption has to be re-examined. Three cases in total, counting the SSTL concept above. Aside from these two concepts, the baffle is usually not integrated. The reason usually given is that the two elements have incompatible accuracy requirements, but that on its own does not settle it: a baffle deployment more accurate than the baffle needs is not a problem, so differing requirements alone would simply mean designing to the tighter one. The binding reason is thermal. The baffle intercepts direct sunlight and albedo, so it heats and fluctuates, and a mirror support sharing that structure inherits the resulting thermo-mechanical dynamics, which is precisely what makes the tighter requirement hard to hold.
...
% "If you are specific enough you will eventually find no literature. The question is how relevant the combination is.
%  Is the VLEO environment really very different from LEO? If so, in what respect?"
% He assumes examples in the 20-30 cm class do exist. VLEO itself is new, so it may well be the interesting angle,
% but the text has to name WHAT is new: the thermal environment, or the atomic oxygen and drag.
% Action: research the LEO vs VLEO difference and rewrite this bullet around the mechanism behind the gap.
    ...
    \item The volume the support structure itself demands between mirror and baffle is not reported. It matters because the baffle has to clear the structure: the more room the mechanism takes there, the larger the baffle becomes, and the stowed volume of the whole system grows with it. An architecture that folds wide at its base, a deployable truss above all, is heavily penalised on this and the penalty does not appear in any figure the literature gives.
...
% "Certainly interesting given the current status. But this alternative can generate a lot of work, so keep something in reserve.
%  If in a few weeks it turns out to be on-axis anyway, that is an opportunity to descope."
% His estimate is that the mission goes on-axis: the central obstruction costs only ~10-11%, an off-axis M2 needs a much larger
% baffle, and both mirrors would have to go freeform, so strong radiometric or optical arguments would be needed to defend it.
% Off-axis is also awkward to carry into the parametric models because it comes in many kinds.
% Action: keep this bullet for now; drop it, and sub-question 2.2 with it, once the instrument design settles on-axis.
...
% TODO (Jasper, 27 Aug 2026) DECISION NEEDED on the framing of the whole question set.
% "You could make it a microsatellite telescope for Earth observation, with PLASMO as the design case. That way the
%  parameter ranges of interest are not necessarily set by the current state of the design, and the question becomes
%  rather more scientific, while keeping a strong case-driven design focus."
% Same point from the research side in the meeting: the thesis should serve the project AND anyone interested in
% deployable optics, which argues for taking parameters as variable inputs from the start rather than running a
% sensitivity analysis afterwards. If adopted, this rewords the main question and sub-question 2.
%
% TODO (open since 26 Aug 2026) RATIFICATION NEEDED, not raised at the review.
% Sub-question 1 as locked on 16 July named a shortlist: "(articulated fold-in with a 3-DOF fine stage; screw-driven
% truss/mast; telescoping carrier; gated inflatable)". That parenthetical was removed on 26 August because naming
% candidates in a research question pre-commits the trade-off. That is a change to a question Jasper agreed, and it did
% not come up on 27 August. Put it to him on 10 September.
